...
class drawgraph():
    ...
    def AddNodes (self, Nodes):
        ...
        #print Node
        for n in nodes:
            l = ','.join(map(str, n.getVals())) + str(n.disc) +  str(n.getDiscValue())
            self.graph.node(str(self.cnode), label = l)
            self.cnode += 1
    ...
    def printGraph (self, filename='kdtree-out'):
        self.graph=gv.Digraph(format='svg')
        self.graph.render(filename='kdtree-out')
...
#    start_time = time.time()
    g1=gv.Digraph (format='svg') #directed graphs
    y=drawgraph()
    tree = kdtree(3)
    tree.insert([3,1,4])
    tree.insert([2,3,7])
    ...
    tree.breadthFirst()
    #y.Print(g1)
    #filename = g1.render(filename='img/kdtree-out')
    #g1.render('kdtree-out.svg', view=True)
    #tree.breadthFirst()
    #tree.breadthFirst()
    graph=drawgraph()
    graph.AddNodes(tree.getList())
    graph.AddEdges(tree.getEdges())
    graph.printGraph()
